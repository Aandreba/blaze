...
use rscl::{prelude::Event, buffer::BufferRect2D};
...
fn gauss () -> Result<()> {
    ...
    let lhs = BufferRect2D::<f32>::new(&[PI,2.,4.,5.,7.,8.], 2, MemAccess::READ_ONLY, false)?;
    let rhs = BufferRect2D::<f32>::new(&[1.,E,3.,4.,5.,6.], 3, MemAccess::READ_ONLY, false)?;
    let mut result = BufferRect2D::<f32>::uninit(3, 3, MemAccess::WRITE_ONLY, false)?;
