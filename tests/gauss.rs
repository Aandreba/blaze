...
use image::{Rgb};
use rscl::{context::SimpleContext, prelude::Result, image::Image2D, buffer::{flags::MemAccess, rect::Rect2D, Buffer}};
...
fn gauss () -> Result<()> {
    ...
    let image = Image2D::<Rgb<f32>>::from_file("test2.jpg", MemAccess::READ_ONLY, false)?;
    let mut result = Buffer::<f32>::new_uninit(image.width()? * image.height()?, MemAccess::WRITE_ONLY, false)?;

    let mut mask_size = 0;
    let mask = create_blur_mask(10., &mut mask_size);
    todo!()
}

fn create_blur_mask (sigma: f32, mask_size: &mut i32) -> Rect2D<f32> {
    *mask_size = f32::ceil(3.0 * sigma) as i32;
