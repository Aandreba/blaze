...
#[test]
fn col_major () {
    let rect = Rect2D::new_col_major(&[1, 2, 3, 4, 5, 6, 7, 8, 9], 3).unwrap();
    assert_eq!(rect.as_slice(), &[1, 4, 7, 2, 5, 8, 3, 6, 9])
}

#[test]
fn transpose () {
    let rect = Rect2D::new(&[1, 2, 3, 4, 5, 6, 7, 8, 9], 3).unwrap();
...
fn boxed () {
    ...
    let rect = Rect2D::from_boxed_slice(slice, 3).unwrap();
...
#[test]
fn zeroed () {
    let rect = Rect2D::<i32>::new_zeroed(3, 3).unwrap();
