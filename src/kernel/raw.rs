...
use crate::{core::*, context::{Context, Global}, buffer::flags::MemAccess};

pub struct RawKernel<C: Context = Global> {
    pub(crate) inner: cl_kernel,
    pub(crate) ctx: C,
    pub(crate) lock: RawFairMutex
}

impl<C: Context> RawKernel<C> {
    #[inline(always)]
...
impl<C: Context> RawKernel<C> {
    ...
    pub fn arg_access_qualifier (&self, idx: u32) -> Result<MemAccess> {
        let flags = self.get_arg_info::<cl_kernel_arg_access_qualifier>(CL_KERNEL_ARG_ACCESS_QUALIFIER, idx)?;
        return Ok(match flags {
            CL_KERNEL_ARG_ACCESS_READ_ONLY => MemAccess::READ_ONLY,
            CL_KERNEL_ARG_ACCESS_WRITE_ONLY => MemAccess::WRITE_ONLY,
            CL_KERNEL_ARG_ACCESS_READ_WRITE => MemAccess::READ_WRITE,
            CL_KERNEL_ARG_ACCESS_NONE => MemAccess::NONE,
            _ => unreachable!()
        })
    }
