...
use core::{
    fmt::{Debug, Display},
    mem::MaybeUninit,
    num::{IntErrorKind, NonZeroU32, NonZeroU64, NonZeroUsize},
    str::FromStr,
};
use opencl_sys::*;
...
impl RawDevice {
    ...
    pub fn atomic_memory_capabilities(&self) -> Result<Option<AtomicCapabilities>> {
        let v = self.get_info_bits::<opencl_sys::cl_device_atomic_capabilities>(
            opencl_sys::CL_DEVICE_ATOMIC_MEMORY_CAPABILITIES,
        )?;
    ...
    pub fn atomic_fence_capabilities(&self) -> Result<Option<AtomicCapabilities>> {
        let v = self.get_info_bits::<opencl_sys::cl_device_atomic_capabilities>(
            opencl_sys::CL_DEVICE_ATOMIC_FENCE_CAPABILITIES,
        )?;
        Ok(AtomicCapabilities::from_bits(v))
    }
    ...
    pub fn device_enqueue_capabilities(&self) -> Result<Option<DeviceEnqueueCapabilities>> {
        let v = self.get_info_bits::<opencl_sys::cl_device_device_enqueue_capabilities>(
            opencl_sys::CL_DEVICE_DEVICE_ENQUEUE_CAPABILITIES,
        )?;
    ...
    pub fn extensions(&self) -> Result<Vec<String>> {
        Ok(self
            .get_info_string(CL_DEVICE_EXTENSIONS)?
            .split_whitespace()
            .map(String::from)
            .collect::<Vec<_>>())
    }
    ...
    pub fn generic_address_space_support(&self) -> Result<bool> {
        let v =
            self.get_info_bits::<cl_bool>(opencl_sys::CL_DEVICE_GENERIC_ADDRESS_SPACE_SUPPORT)?;
        Ok(v != 0)
    }
    ...
    pub fn image_max_array_size(&self) -> Result<Option<NonZeroUsize>> {
        self.get_info_bits(opencl_sys::CL_DEVICE_IMAGE_MAX_ARRAY_SIZE)
    ...
    pub fn image_max_buffer_size(&self) -> Result<Option<NonZeroUsize>> {
        self.get_info_bits(opencl_sys::CL_DEVICE_IMAGE_MAX_BUFFER_SIZE)
            .map(NonZeroUsize::new)
    }
    ...
    pub fn image_pitch_alignment(&self) -> Result<Option<NonZeroU32>> {
        self.get_info_bits(opencl_sys::CL_DEVICE_IMAGE_PITCH_ALIGNMENT)
            .map(NonZeroU32::new)
    }
    ...
    pub fn image2d_max_height(&self) -> Result<Option<NonZeroUsize>> {
        self.get_info_bits::<usize>(CL_DEVICE_IMAGE2D_MAX_HEIGHT)
            .map(NonZeroUsize::new)
    }
    ...
    pub fn image2d_max_width(&self) -> Result<Option<NonZeroUsize>> {
        self.get_info_bits::<usize>(CL_DEVICE_IMAGE2D_MAX_WIDTH)
            .map(NonZeroUsize::new)
    }
    ...
    pub fn image3d_max_depth(&self) -> Result<Option<NonZeroUsize>> {
        self.get_info_bits::<usize>(CL_DEVICE_IMAGE3D_MAX_DEPTH)
            .map(NonZeroUsize::new)
    }
    ...
    pub fn image3d_max_height(&self) -> Result<Option<NonZeroUsize>> {
        self.get_info_bits::<usize>(CL_DEVICE_IMAGE3D_MAX_HEIGHT)
            .map(NonZeroUsize::new)
    }
    ...
    pub fn image3d_max_width(&self) -> Result<Option<NonZeroUsize>> {
        self.get_info_bits::<usize>(CL_DEVICE_IMAGE3D_MAX_WIDTH)
            .map(NonZeroUsize::new)
    }
    ...
    pub fn local_mem_size(&self) -> Result<NonZeroU64> {
        unsafe {
            Ok(NonZeroU64::new_unchecked(
                self.get_info_bits::<u64>(CL_DEVICE_LOCAL_MEM_SIZE)?,
            ))
        }
    }
    ...
    pub fn max_compute_units(&self) -> Result<NonZeroU32> {
        unsafe {
            Ok(NonZeroU32::new_unchecked(
                self.get_info_bits::<u32>(CL_DEVICE_MAX_COMPUTE_UNITS)?,
            ))
        }
    }
    ...
    pub fn max_constant_args(&self) -> Result<NonZeroU32> {
        unsafe {
            Ok(NonZeroU32::new_unchecked(
                self.get_info_bits::<u32>(CL_DEVICE_MAX_CONSTANT_ARGS)?,
            ))
        }
    }
    ...
    pub fn max_constant_buffer_size(&self) -> Result<NonZeroU64> {
        unsafe {
            Ok(NonZeroU64::new_unchecked(self.get_info_bits::<u64>(
    ...
    pub fn max_global_variable_size(&self) -> Result<Option<NonZeroUsize>> {
        self.get_info_bits(opencl_sys::CL_DEVICE_MAX_GLOBAL_VARIABLE_SIZE)
            .map(NonZeroUsize::new)
    }
    ...
    pub fn max_mem_alloc_size(&self) -> Result<NonZeroU64> {
        unsafe {
            Ok(NonZeroU64::new_unchecked(
                self.get_info_bits::<u64>(CL_DEVICE_MAX_MEM_ALLOC_SIZE)?,
            ))
        }
    }
    ...
    pub fn max_num_sub_groups(&self) -> Result<Option<NonZeroU32>> {
        self.get_info_bits(opencl_sys::CL_DEVICE_MAX_NUM_SUB_GROUPS)
            .map(NonZeroU32::new)
    }
    ...
    pub fn max_on_device_events(&self) -> Result<Option<NonZeroU32>> {
        self.get_info_bits(opencl_sys::CL_DEVICE_MAX_ON_DEVICE_EVENTS)
            .map(NonZeroU32::new)
    }
    ...
    pub fn max_on_device_queues(&self) -> Result<Option<NonZeroU32>> {
        self.get_info_bits(opencl_sys::CL_DEVICE_MAX_ON_DEVICE_QUEUES)
            .map(NonZeroU32::new)
    }
    ...
    pub fn max_parameter_size(&self) -> Result<NonZeroUsize> {
        unsafe {
            Ok(NonZeroUsize::new_unchecked(
                self.get_info_bits::<usize>(CL_DEVICE_MAX_PARAMETER_SIZE)?,
            ))
        }
    }
    ...
    pub fn max_pipe_args(&self) -> Result<Option<NonZeroU32>> {
        self.get_info_bits(opencl_sys::CL_DEVICE_MAX_PIPE_ARGS)
            .map(NonZeroU32::new)
    }
    ...
    pub fn max_read_image_args(&self) -> Result<Option<NonZeroU32>> {
        self.get_info_bits::<u32>(CL_DEVICE_MAX_READ_IMAGE_ARGS)
            .map(NonZeroU32::new)
    }
    ...
    pub fn max_read_write_image_args(&self) -> Result<Option<NonZeroU32>> {
        self.get_info_bits::<u32>(opencl_sys::CL_DEVICE_MAX_READ_IMAGE_ARGS)
            .map(NonZeroU32::new)
    }
    ...
    pub fn max_samplers(&self) -> Result<Option<NonZeroU32>> {
        self.get_info_bits::<u32>(CL_DEVICE_MAX_SAMPLERS)
            .map(NonZeroU32::new)
    }
    ...
    pub fn max_work_group_size(&self) -> Result<NonZeroUsize> {
        unsafe {
            Ok(NonZeroUsize::new_unchecked(
                self.get_info_bits::<usize>(CL_DEVICE_MAX_WORK_GROUP_SIZE)?,
            ))
        }
    }
    ...
    pub fn max_work_item_dimensions(&self) -> Result<NonZeroU32> {
        unsafe {
            Ok(NonZeroU32::new_unchecked(self.get_info_bits::<u32>(
    ...
    pub fn max_work_item_sizes(&self) -> Result<Vec<NonZeroUsize>> {
        ...
        let len = n
            ...
            .expect("Integer multiplication oveflow. Too many work items to fit in a vector");
        unsafe {
            clGetDeviceInfo(
                self.id(),
                CL_DEVICE_MAX_WORK_ITEM_SIZES,
                len,
                max_work_item_sizes.as_mut_ptr().cast(),
                core::ptr::null_mut(),
            );
    ...
    pub fn max_write_image_args(&self) -> Result<Option<NonZeroU32>> {
        self.get_info_bits::<u32>(CL_DEVICE_MAX_WRITE_IMAGE_ARGS)
            .map(NonZeroU32::new)
    }
    ...
    pub fn non_uniform_work_group_support(&self) -> Result<bool> {
        let v =
            self.get_info_bits::<cl_bool>(opencl_sys::CL_DEVICE_NON_UNIFORM_WORK_GROUP_SUPPORT)?;
        Ok(v != 0)
    }
    ...
    pub fn partition_affinity_domain(&self) -> Result<Option<AffinityDomain>> {
        let v = self.get_info_bits::<opencl_sys::cl_device_affinity_domain>(
            opencl_sys::CL_DEVICE_PARTITION_PROPERTIES,
        )?;
    ...
    pub fn partition_type(&self) -> Result<Option<PartitionProperty>> {
        let v = self.get_info_array::<opencl_sys::cl_device_partition_property>(
            opencl_sys::CL_DEVICE_PARTITION_TYPE,
        )?;
    ...
    pub fn pipe_max_active_reservations(&self) -> Result<Option<NonZeroU32>> {
        self.get_info_bits(opencl_sys::CL_DEVICE_PIPE_MAX_ACTIVE_RESERVATIONS)
            .map(NonZeroU32::new)
    }
    ...
    pub fn pipe_max_packet_size(&self) -> Result<Option<NonZeroU32>> {
        self.get_info_bits(opencl_sys::CL_DEVICE_PIPE_MAX_PACKET_SIZE)
            .map(NonZeroU32::new)
    }
    ...
    pub fn platform(&self) -> Result<RawPlatform> {
        ...
        unsafe { return RawPlatform::from_id(id).ok_or_else(|| ErrorKind::InvalidPlatform.into()) }
    }
    ...
    pub fn queue_on_device_properties(&self) -> Result<CommandQueueProperties> {
        let v = self.get_info_bits::<cl_command_queue_properties>(
            opencl_sys::CL_DEVICE_QUEUE_ON_DEVICE_PROPERTIES,
        )?;
        Ok(CommandQueueProperties::from_bits(v))
    }
    ...
    pub fn queue_on_device_max_size(&self) -> Result<Option<NonZeroU32>> {
        self.get_info_bits(opencl_sys::CL_DEVICE_QUEUE_ON_DEVICE_MAX_SIZE)
            .map(NonZeroU32::new)
    }
    ...
    pub fn queue_on_device_preferred_size(&self) -> Result<Option<NonZeroU32>> {
        self.get_info_bits(opencl_sys::CL_DEVICE_QUEUE_ON_DEVICE_PREFERRED_SIZE)
            .map(NonZeroU32::new)
    }
    ...
    pub fn queue_on_host_properties(&self) -> Result<CommandQueueProperties> {
        let v = self.get_info_bits::<cl_command_queue_properties>(
            opencl_sys::CL_DEVICE_QUEUE_ON_HOST_PROPERTIES,
        )?;
        Ok(CommandQueueProperties::from_bits(v))
    }
    ...
    pub fn sub_group_independent_forward_progress(&self) -> Result<bool> {
        let v = self.get_info_bits::<cl_bool>(
            opencl_sys::CL_DEVICE_SUB_GROUP_INDEPENDENT_FORWARD_PROGRESS,
        )?;
        Ok(v != 0)
    }
    ...
    pub fn work_group_collective_functions_support(&self) -> Result<bool> {
        let v = self.get_info_bits::<cl_bool>(
            opencl_sys::CL_DEVICE_WORK_GROUP_COLLECTIVE_FUNCTIONS_SUPPORT,
        )?;
        Ok(v != 0)
    }
    ...
    pub fn create_sub_devices(&self, prop: PartitionProperty) -> Result<Vec<RawDevice>> {
        ...
        let mut len = 0;
        unsafe {
            tri!(opencl_sys::clCreateSubDevices(
                self.id(),
                prop.as_ptr(),
                0,
                core::ptr::null_mut(),
    ...
    #[docfg(feature = "cl2_1")]
    #[inline(always)]
    pub fn set_default_command_queue(
        &self,
        ctx: crate::context::RawContext,
        queue: RawCommandQueue,
    ) -> Result<()> {
        unsafe {
            tri!(opencl_sys::clSetDefaultDeviceCommandQueue(
                ctx.id(),
                self.id(),
                queue.id()
            ));
    ...
    pub fn device_and_host_timer(&self) -> Result<(std::time::SystemTime, std::time::SystemTime)> {
        ...
        let device = std::time::UNIX_EPOCH
            .checked_add(std::time::Duration::from_nanos(device))
            .unwrap();
        let host = std::time::UNIX_EPOCH
            .checked_add(std::time::Duration::from_nanos(host))
            .unwrap();
        Ok((device, host))
    }
    ...
    pub fn host_clock_nanos(&self) -> Result<u64> {
        let mut host = 0;
        unsafe { tri!(clGetHostTimer(self.id(), std::ptr::addr_of_mut!(host))) }
...
impl PartitionProperty {
    pub fn from_slice(bits: &[opencl_sys::cl_device_partition_property]) -> Option<Self> {
        ...
        match unsafe { *bits.get_unchecked(0) } {
            ...
            opencl_sys::CL_DEVICE_PARTITION_BY_AFFINITY_DOMAIN => {
                Some(Self::AffinityDomain(unsafe {
                    core::mem::transmute(bits[1] as u64)
                }))
            }
            opencl_sys::CL_DEVICE_PARTITION_BY_COUNTS => {
                ...
                for i in 1..bits.len() {
                    ...
                    match bits[i] {
                        ...
                        v @ 1..=MAX_COUNT => unsafe {
                            result.push(NonZeroU32::new_unchecked(v as u32))
                        },
                        _ => return None,
                    }
                }
    ...
    pub fn to_bits(&self) -> Box<[opencl_sys::cl_device_partition_property]> {
        match self {
            Self::Equally(n) => Box::new([
                opencl_sys::CL_DEVICE_PARTITION_EQUALLY,
                opencl_sys::cl_device_partition_property::try_from(*n).unwrap(),
                0,
            ]) as Box<_>,
            Self::AffinityDomain(x) => Box::new([
                opencl_sys::CL_DEVICE_PARTITION_BY_AFFINITY_DOMAIN,
                opencl_sys::cl_device_partition_property::try_from(*x as u64).unwrap(),
                0,
            ]) as Box<_>,
            Self::Counts(x) => {
                ...
                unsafe {
                    ...
                    for i in 0..x.len() {
                        result[1 + i].write(
                            opencl_sys::cl_device_partition_property::try_from(x[i].get()).unwrap(),
                        );
                    }
