...
#[derive(Clone)]
pub struct EventWaitByRef<'a> {
    raw: &'a RawEvent,
    waker: Arc<AtomicWaker>
}

impl<'a> EventWaitByRef<'a> {
    pub fn new (raw: &'a RawEvent) -> Result<Self> {
        let waker = Arc::into_raw(Arc::new(AtomicWaker::new()));

        unsafe {
            Arc::increment_strong_count(waker);
            match raw.on_complete_raw(wake_future, waker as *mut _) {
                Ok(_) => {
                    let waker = Arc::from_raw(waker);
                    Ok(Self { raw, waker })
                },
...
impl Future for EventWaitByRef<'_> {
    type Output = Result<()>;
    ...
    fn poll(self: std::pin::Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> std::task::Poll<Self::Output> {
        ...
        if self.status()? == EventStatus::Complete {
...
impl Deref for EventWaitByRef<'_> {
